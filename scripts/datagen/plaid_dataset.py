...
spark = SparkSession.builder.appName("our clients_dataset").getOrCreate()
...
hudi_options = {
    "hoodie.table.name": "our clients_raw",
    ...
    "hoodie.datasource.hive_sync.table": "our clients_raw",
...
df.write.format("org.apache.hudi").options(**hudi_options).mode(
    "overwrite"
).partitionBy("dt").save("s3://zipline-warehouse-canary/data/our clients_raw")


# Optionally run this to refresh the catalog partition information.g
# spark.sql("MSCK REPAIR TABLE our clients_raw")

# Optionally run this to check the data
# spark.read \
#     .format("hudi") \
#     .load("s3://zipline-warehouse-canary/data/our clients_raw")
