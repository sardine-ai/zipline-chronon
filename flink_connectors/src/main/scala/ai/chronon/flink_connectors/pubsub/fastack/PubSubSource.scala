...
/** The deserialization schema describes how to turn the PubsubMessages into data types (Java/Scala
  ...
  */
trait PubSubDeserializationSchema[T] extends Serializable with ResultTypeQueryable[T] {
  ...
  /** Initialization method for the schema. It is called before the actual working methods {@link
    ...
    */
  @throws[Exception]
  def open(context: DeserializationSchema.InitializationContext): Unit = {}
  ...
  /** Deserializes a PubsubMessage.
    ...
    */
  @throws[Exception]
  def deserialize(message: PubsubMessage): T
  ...
  /** Deserializes the PubSub record.
    ...
    */
  @throws[Exception]
  def deserialize(message: PubsubMessage, out: Collector[T]): Unit = {
    val deserialized = deserialize(message)
    if (deserialized != null) out.collect(deserialized)
  }
}
