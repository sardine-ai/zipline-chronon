...
object TableDependencies {
  ...
  def fromSource(source: api.Source,
    ...
    val startOffset: Window = (source.dataModel, maxWindowOpt, lagOpt) match {
      ...
      case (DataModel.EVENTS, None, _) => null // when start offset is null, we won't try to fill anything

      case (DataModel.EVENTS, Some(aggregationWindow), None) =>
        lagOpt.map(lag => WindowUtils.plus(aggregationWindow, lag)).getOrElse(aggregationWindow)
    }
