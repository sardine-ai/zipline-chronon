        ...
        for gb_name in join["group_bys"]:
            if gb_name in gb_index:
                ...
                if len(join["_events_driver"]) > 0:
                    gb_index[gb_name]["join_event_driver"].append(
                        join["_events_driver"][0]
                    )
...
def author_name_email(file, exclude=None):
    ...
    if file not in file_to_author:
        for file_path, auth_str in git_info([file], exclude).items():
            file_to_author[file_path] = auth_str.split("/")[-2:]
...
def events_without_topics(output_file=None, exclude_commit_message=None):
    ...
    def is_events_without_topics(entry):
        ...
        if found:
            file = (
                entry["json_file"]
                if os.path.exists(entry["json_file"])
                else entry["file"]
            )
            producer_name, producer_email = author_name_email(
                file, exclude_commit_message
            )
