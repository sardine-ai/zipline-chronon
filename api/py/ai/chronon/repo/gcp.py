...
class GcpRunner(Runner):
    ...
    def download_zipline_dataproc_jar(destination_dir: str, customer_id: str, jar_name: str):
        ...
        are_identical = GcpRunner.compare_gcs_and_local_file_hashes(bucket_name, source_blob_name,
        ...
        if are_identical:
            ...
            GcpRunner.download_gcs_blob(bucket_name, source_blob_name,
                                destination_path)
        return destination_path


    def generate_dataproc_submitter_args(self, user_args: str, job_type: DataprocJobType = DataprocJobType.SPARK,
        ...
        for source_file in local_files_to_upload:
            ...
            gcs_files.append(GcpRunner.upload_gcs_blob(
        ...
        if len(command_list) > 1:
            ...
            if dataproc_submitter_logs:
                ...
                try:
                    ...
                    ------------------------------------------------------------------------------------>
                    """)
                    check_call(f"gcloud dataproc jobs wait {job_id} --region={self.get_gcp_region_id()}")
                except Exception:
