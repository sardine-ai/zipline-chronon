...
class ConfigDetails:
    ...
    def __init__(self, obj):
        ...
        try:
            obj_type = next(k for k, v in FOLDER_NAME_TO_CLASS.items() if isinstance(obj, v))
        ...
        if path_parts[0] != obj_type:
            raise ValueError(f"Expected module path to start with '{obj_type}', got {self.module}")
...
def get_chronon_root(filepath):
    ...
    raise ValueError(f"{filepath} is not within a valid Chronon root directory, containing {target_dirs} subdirs.")    
