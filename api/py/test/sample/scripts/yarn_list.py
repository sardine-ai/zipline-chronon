...
def get_active_applications(
    ...
    emr_application_list_cmd = (
        "{yarn_executable} --config {hadoop_conf} application -list".format(
    ...
    for app_listing in output.strip().split("\n")[2:]:
        ...
        job = {
            ...
            "kill_cmd": "{yarn_executable} --config {hadoop_conf} application -kill {app_id}".format(
                **locals()
            ),
        }
