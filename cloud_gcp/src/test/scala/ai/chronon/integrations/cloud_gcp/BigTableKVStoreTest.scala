...
import ai.chronon.integrations.cloud_gcp.GcpApiImpl.UseBulkReadRows
...
class BigTableKVStoreTest extends AnyFlatSpec with BeforeAndAfter with TableDrivenPropertyChecks {
  ...
  it should "blob data round trip" in {
    forAll(createConfigurations()) { (description, kvStore) =>
      withClue(s"Testing with $description: ") {
        val dataset = "models"
        kvStore.create(dataset)
  ...
  it should "multiget failures" in {
    ...
    when(mockBatcher.closeAsync()).thenReturn(ApiFutures.immediateCancelledFuture[Void]())
  ...
  def createConfigurations(): TableFor2[String, BigTableKVStoreImpl] = {
    Table(
      ("description", "kvStore"),
      ("multiGet - bulk read enabled enabled", new BigTableKVStoreImpl(dataClient, Some(adminClient), None, Map(UseBulkReadRows -> "true"))),
      ("multiGet - with bulk read rows disabled", new BigTableKVStoreImpl(dataClient, Some(adminClient), None, Map(UseBulkReadRows -> "false")))
    )
  }
}
