...
/**
  ...
  * 2) Timeseries data - This is either our batch IRs or streaming ETLed events / tiles for feature fetching. It also
  * includes drift / skew time summaries.
  *
  * We have multi use-case tables for the _BATCH and _STREAMING time series event / tile data.
  * To ensure that data from different groupBys are isolated from each other, we prefix the key with the dataset name:
  * Row key: dataset#key
  *
  * In case of time series data that is likely to see many data points per day (e.g. tile_summaries, streaming events / tiles), we
  ...
  *
  * This row key structure looks like:
  * Row key: dataset#key#timestamp_rounded_to_day
  *
...
class BigTableKVStoreImpl(dataClient: BigtableDataClient,
  ...
  private def isTimeSeriesDataset(dataset: String): Boolean =
    dataset.endsWith("_STREAMING") || dataset.endsWith("SUMMARIES")
