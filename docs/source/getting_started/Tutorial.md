...
To get started with the Chronon, all you need to do is download the [docker-compose.yml](https://github.com/zipline-ai/chronon/blob/main/docker-compose.yml) file and run it locally:
...
Fabricated raw data is included in the [data](https://github.com/zipline-ai/chronon/blob/main/api/python/test/sample/data) directory. It includes four tables:
...
We perform a similar set of aggregations on returns data in the [returns GroupBy](https://github.com/zipline-ai/chronon/blob/main/api/python/test/sample/group_bys/quickstart/returns.py). The code is not included here because it looks similar to the above example.
...
As discussed in the introductory sections of the main project [README](https://github.com/zipline-ai/chronon?tab=readme-ov-file#platform-features), one of Chronon's core guarantees is online/offline consistency. This means that the data that you use to train your model (offline) matches the data that the model sees for production inference (online).
...
For a more detailed view into the benefits of using Chronon, see [Benefits of Chronon documentation](https://github.com/zipline-ai/chronon/tree/main?tab=readme-ov-file#benefits-of-chronon-over-other-approaches).
