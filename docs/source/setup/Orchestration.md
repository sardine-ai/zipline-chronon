...
See the [Airflow Directory](https://github.com/zipline-ai/chronon/tree/main/airflow) for initial boilerplate code. 

The files in this directory can be used to create the following Chronon Airflow DAGs.

1. GroupBy DAGs, created by [group_by_dag_constructor.py](https://github.com/zipline-ai/chronon/tree/main/airflow/group_by_dag_constructor.py):
...
2. Join DAGs, created by [join_dag_constructor.py](https://github.com/zipline-ai/chronon/tree/main/airflow/join_dag_constructor.py):
...
3. Staging Query DAGs, created by [staging_query_dag_constructor.py](https://github.com/zipline-ai/chronon/tree/main/airflow/staging_query_dag_constructor.py):
   1. `chronon_staging_query_{team_name}`: One DAG per team that creates daily jobs for each Staging Query for the team.  
4. Online/Offline Consistency Check DAGs, created by [online_offline_consistency_dag_constructor.py](https://github.com/zipline-ai/chronon/tree/main/airflow/online_offline_consistency_dag_constructor.py):
...
1. Set your configuration variables in [constants.py](https://github.com/zipline-ai/chronon/tree/main/airflow/constants.py).
2. Implement the `get_kv_store_upload_operator` function in [helpers.py](https://github.com/zipline-ai/chronon/tree/main/airflow/helpers.py). **This is only required if you want to use Chronon online serving**.


## Alternate Integrations


While Airflow is currently the most well-supported integration, there is no reason why you couldn't choose a different orchestration engine to power the above flows. If you're interested in such an integration and you think that the community might benefit from your work, please consider [contributing](https://github.com/zipline-ai/chronon/blob/main/CONTRIBUTING.md) back to the project.
