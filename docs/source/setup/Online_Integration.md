...
If you'd to start with an example, please refer to the [MongoDB Implementation in the Quickstart Guide](https://github.com/zipline/chronon/tree/main/quickstart/mongo-online-impl/src/main/scala/ai/chronon/quickstart/online). This provides a complete working example of how to integrate Chronon with MongoDB. 

## Components

**KVStore**: The biggest part of the API implementation is the [KVStore](https://github.com/zipline/chronon/blob/main/online/src/main/scala/ai/chronon/online/Api.scala#L43).
...
2. `multiGet`: which takes a `Seq` of [`GetRequest`](https://github.com/zipline/chronon/blob/main/online/src/main/scala/ai/chronon/online/Api.scala#L33) and converts them into a `Future[Seq[GetResponse]]` by querying the underlying KVStore.
3. `multiPut`: which takes a `Seq` of [`PutRequest`](https://github.com/zipline/chronon/blob/main/online/src/main/scala/ai/chronon/online/Api.scala#L38) and converts them into `Future[Seq[Boolean]]` (success/fail) by attempting to insert them into the underlying KVStore.
...
See the [MongoDB example here](https://github.com/zipline/chronon/blob/main/quickstart/mongo-online-impl/src/main/scala/ai/chronon/quickstart/online/MongoKvStore.scala).
...
See the [Quickstart example here](https://github.com/zipline/chronon/blob/main/quickstart/mongo-online-impl/src/main/scala/ai/chronon/quickstart/online/QuickstartMutationDecoder.scala).


**API:** The main API that requires implementation is [API](https://github.com/zipline/chronon/blob/main/online/src/main/scala/ai/chronon/online/Api.scala#L151). This combines the above implementations with other client and logging configuration.

[ChrononMongoOnlineImpl](https://github.com/zipline/chronon/blob/main/quickstart/mongo-online-impl/src/main/scala/ai/chronon/quickstart/online/ChrononMongoOnlineImpl.scala) Is an example implemenation of the API.
