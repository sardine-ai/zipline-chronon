package ai.chronon.orchestration.test.agent

import ai.chronon.api.JobStatusType
import ai.chronon.integrations.cloud_gcp.BigTableKVStoreImpl
import ai.chronon.orchestration.agent.JobStore
import ai.chronon.orchestration.test.utils.TestUtils
...
class BigTableKVJobStoreSpec extends AnyFlatSpec with Matchers with BeforeAndAfter {
  ...
  before {
    ...
    // Create JobStore
    jobStore = JobStore.createWithKVStore(kvStore)
  }
