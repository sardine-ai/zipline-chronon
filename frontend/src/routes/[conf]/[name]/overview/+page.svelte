<script lang="ts">
	...
	type DagreData = ComponentProps<typeof Dagre>['data'];
	...
	let graph = $state<ComponentProps<typeof Dagre>['graph'] | undefined>(undefined);
	...
	function isUpstream(node: CustomNode) {
		if (graph && hoveredNode) {
			const upstream = dagreAncestors(graph, hoveredNode.id) as unknown as string[];
	...
	function isDownstream(node: CustomNode) {
		if (graph && hoveredNode) {
			const downstream = dagreDescendants(graph, hoveredNode.id) as unknown as string[];
	...
	let selectedDialogTab = $state('overview');
</script>

<CollapsibleSection label="Details" open class="mt-5 mb-6">
		...
		>
			{#snippet children({ context })}
							...
							<Html>
								<div class="nodes">
									{#each nodes as _node (_node.label)}
										...
										{@const { primary, secondary } = getEntityLabel(node.label)}
				...
				<Svg ignoreTransform pointerEvents={false}>
					...
					<Group x={context.width - legendWidth} y={context.height - legendHeight}>
						<Rect
							x={0}
							y={0}
							width={legendWidth}
				...
				{#if !hideTooltip}
					<Tooltip.Root {...tooltipProps.root} contained="window" xOffset={0} yOffset={30}>
...
>
	{#if selectedNode}
		{@const { primary, secondary } = getEntityLabel(selectedNode.label)}
		...
		<div class="w-[1000px] max-w-[90vw] px-6">
			<header class="sticky top-0 z-10 bg-surface-100 pt-4 mb-3">
				...
				<ToggleGroup
					bind:value={selectedDialogTab}
					variant="underline"
					classes={{
						root: '[--color-primary:var(--color-primary-800)] -mx-7',
						options: 'justify-start h-9 px-7',
						option: 'flex gap-2'
					}}
				>
					<ToggleOption value="overview">Overview</ToggleOption>
					<ToggleOption value="details">Details</ToggleOption>
				</ToggleGroup>
			</header>

			<div class="overflow-auto">
				{#if selectedDialogTab === 'overview'}
					<EntityProperties entity={selectedNode.value.conf as IJoinArgs} includeUpstream />
				{:else if selectedDialogTab === 'details'}
					<div class="border py-2 px-4 mt-2 rounded-md bg-neutral-200">
						<Inspect
