...
object FlinkJob {
  ...
  val CheckPointInterval = 10.seconds
  ...
  val CheckpointTimeout = 5.minutes
  ...
  // Trigger watermarks frequently - useful for low-latency scenarios to help close windows, move time forward
  val AutoWatermarkInterval = 100.milliseconds
  ...
  def main(args: Array[String]): Unit = {
    ...
    env.getConfig.enableForceKryo() // use kryo for complex types that Flink's default ser system doesn't support (e.g case classes)
