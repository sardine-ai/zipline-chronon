...
class ShortNamesTest extends AnyFlatSpec {
  ...
  it should "test modular with short names" in {
    ...
    val jp1 = Builders.JoinPart(groupBy = groupBy,
    ...
    val joinConf: ai.chronon.api.Join = Builders.Join(
      ...
      joinParts = Seq(jp1, jp2, Builders.JoinPart(groupBy = bootstrapGroupBy, useLongNames = false)),
    ...
    val metaDataJp1 = new api.MetaData()
    ...
    finalJoinJob.run()
    logger.info(s"SCHEMA == ${tableUtils.sql(s"SELECT * FROM $mergeJobOutputTable").schema.pretty}")
