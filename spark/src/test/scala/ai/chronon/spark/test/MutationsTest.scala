...
class MutationsTest extends AnyFlatSpec with TaggedFilterSuite {
  ...
  it should "test with generated data" in {
    ...
    val (_, maxDs) = mutationsDf.range[String](tableUtils.defaultPartitionColumn)
    val (minDs, _) = snapshotDf.range[String](tableUtils.defaultPartitionColumn)
    val leftDf = DataFrameGen
      .events(spark, events, 100, 15)
      .drop()
      .withShiftedPartition(tableUtils.defaultPartitionColumn, -1)
    val testNamespace = namespace(suffix)
    tableUtils.sql(s"CREATE DATABASE IF NOT EXISTS $testNamespace")
