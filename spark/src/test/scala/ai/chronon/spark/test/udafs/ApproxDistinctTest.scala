...
class ApproxDistinctTest extends AnyFlatSpec with Matchers {
  ...
  "ApproxDistinct.columnCardinality" should "correctly estimate cardinality for various column types" in {
    val spark = SparkSession.builder().appName("ApproxDistinctTest").master("local[*]").getOrCreate()
  ...
  it should "handle null values correctly" in {
    val spark = SparkSession.builder().appName("ApproxDistinctNullTest").master("local[*]").getOrCreate()
    import spark.implicits._
    ...
    val df: DataFrame = Seq(
      ...
      (1, "A", null),
      (4, "D", 4.0)
    ).toDF("int_col", "string_col", "double_col")
  ...
  it should "handle empty DataFrame" in {
    val spark = SparkSession.builder().appName("ApproxDistinctEmptyTest").master("local[*]").getOrCreate()
    import spark.implicits._
