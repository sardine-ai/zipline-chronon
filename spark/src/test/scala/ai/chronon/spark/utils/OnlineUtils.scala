...
import ai.chronon.api.{Accuracy, Constants, DataModel, TilingUtils}
import ai.chronon.online.KVStore
import ai.chronon.online.serde.AvroConversions
import ai.chronon.spark.{GenericRowHandler, GroupByUpload}
...
object OnlineUtils {
  ...
  def putStreaming(session: SparkSession,
    ...
    inputModified.cache()
    println("~~~ Streaming data to be written to KV Store ~~~")
    inputModified.show(10, truncate = false)
    inputModified.printSchema()
